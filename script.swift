    ...
    func getInfo() -> USBDevice? {
        ...
        if let dict = umDict?.takeRetainedValue() as? [String: Any] {
         //to show all properties available
            ...
            if let serialNumber = dict[kUSBSerialNumberString] as? String {
                _serialNr = serialNumber
            }


            if let vendorName = dict["USB Vendor Name"] as? String {
        ...
        return USBDevice(id: did, vendorId: vid, productId: pid, name: name, locationId: lid, vendorName: _vendorName, serialNr: _serialNr, bsdPath: bsdPath, deviceInterfacePtrPtr: deviceInterfacePtrPtr, plugInInterfacePtrPtr: plugInInterfacePtrPtr)
...
let example = Example()
usbWatcher = USBWatcher(delegate: example)
